# velodb-mcp-server Project Audit Report
...
- `src/server.py:327-329` `_get_workspace_from_request` does not validate the `?workspace=` value; it flows into `VeloDBStore(workspace=ws)` (`src/store/store.py:103-107`) and is interpolated via f-strings into `CREATE/DROP/USE/SELECT` SQL (e.g. `store.py:164, 476-496`) and file paths (`src/store/watcher.py:277`).
...
- `src/auth/guard.py:33-36` `check_tool_access` always allows; any valid VeloDB user can call admin tools such as `reload_semantic_layer`; the audit log's `client_id` is always `None`.
...
- **Fix**: pick one — either wire it up (parse the config, actually validate scopes/allow-list), or delete it and state clearly in the docs that "VeloDB credentials are the only authentication".
...
- `src/core/connection.py:43-52`: the aiomysql pool has no `pool_recycle`/ping; the `pool_idle_timeout` config (`loader.py:75`) is read but never used. After VeloDB `wait_timeout` drops idle connections, the first query always errors (a classic intermittent production failure).
...
- **Apache-2.0 compliance gaps**: `src/metricflow/` (three upstream v0.209.0 packages merged and rewritten) has no LICENSE/NOTICE/copyright headers; modifications were made (`sql/render/velodb.py`, `msi_pydantic_shim.py`, etc.) without the notices required by §4(b); the dist tarball is effectively redistributing this code.
...
5. **Local patch bug**: `src/metricflow/.../project_configuration.py:48` falls back to `version("mcp-server")`, but the dist name is `velodb-mcp-server`, so `dsi_package_version` is always `0.0.0` and gets written into the manifest.

### Security / sessions
6. **WebUI sessions store VeloDB passwords in plaintext**: `server.py:1159-1165` — an in-memory dict stores plaintext passwords with a 24h TTL and no capacity limit; the cookie (:1180-1184) lacks the `Secure` flag; the default `mcp_host=0.0.0.0` serves plaintext HTTP.
7. **No brute-force protection**: `/mcp/web/login` and Bearer validation have no rate limiting; CredentialCache only caches successes, so every failure hits VeloDB = an online password-guessing channel.
8. **Hard-coded `user == "admin"` as the sole administrator check** (`server.py:416`), decoupled from VeloDB's actual privilege system.
...
18. **README deployment instructions are wrong**: `README.md:73-74` tells users to `cd velodb-mcp-server`, but the tarball's top-level directory is `velodb-mcp-server/`.
...
20. **pyproject.toml placeholder**: `description = "Add your description here"`; no package-data declaration, and `src/skills/velodb-mcp-skill.md` is read via `importlib.resources` (`server.py:541`) — if packaging omits it, `get_query_guide` fails silently.
...
- build.sh mode 2 does not validate the `VELODB_MCP_SYSTEM_PYTHON` version, while the slimming path hard-codes `python3.10`; pointing to 3.11/3.13 silently breaks.
...
11. Single source of truth for the version number (recommend build.sh reads pyproject); fix README `cd velodb-mcp-server`; sync README/DESIGN with the single-package build and 4 platforms.
...
*This report is based on a static, read-only review and did not connect to a real VeloDB instance to verify runtime behavior; entries marked "suspected" should be reproduced and confirmed before fixing.*

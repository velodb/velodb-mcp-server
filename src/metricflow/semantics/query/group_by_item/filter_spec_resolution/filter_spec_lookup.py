...
from metricflow.semantic_interfaces.call_parameter_sets import (
    ...
    def _checked_resolution(self, resolved_spec_lookup_key: ResolvedSpecLookUpKey) -> FilterSpecResolution:
        ...
        if len(resolutions) == 0:
            raise RuntimeError(
                ...
                f"{mf_indent(mf_pformat(resolved_spec_lookup_key))}\n\n"
                f"but did not find any. All resolutions are:\n\n"
                f"{mf_indent(mf_pformat(self.spec_resolutions))}"
            )

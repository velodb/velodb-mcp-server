...
from metricflow.semantic_interfaces.protocols import SemanticManifest
from metricflow.semantic_interfaces.type_enums.time_granularity import TimeGranularity
from metricflow.semantics.errors.error_classes import SemanticManifestConfigurationError
...
from metricflow.semantics.toolkit.cache.lru_cache import typed_lru_cache
...
class TimeSpineSource:
    ...
    def build_standard_time_spine_sources(
        ...
        time_spine_sources = {
            time_spine.primary_column.time_granularity: TimeSpineSource(
                sql_table=SqlTable.from_string(time_spine.node_relation.relation_name),
        ...
        for legacy_time_spine in legacy_time_spines:
            if legacy_time_spine.grain not in time_spine_sources:
                time_spine_sources[legacy_time_spine.grain] = TimeSpineSource(
                    sql_table=SqlTable.from_string(legacy_time_spine.location),
    ...
    def data_set_description(self) -> str:
        ...
        return f"Read From Time Spine '{self.sql_table.table_name}'"
